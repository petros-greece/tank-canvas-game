class WorldBuilder {
    ...
    giveTeamOfObjects(builderOpts, objectOpts = {}) {
        const objects = [];
        ...
        const halfWidth = objectOpts.width / 2 || 0;  // Assuming object has width property
        const halfHeight = objectOpts.height / 2 || 0; // Assuming object has height property
        ...
        for (let i = 0; i < num; i++) {
            ...
            dx += sx; // Increase dx after each loop
            dy += sy; // Increase dy after each loop
            ...
            // Increment rotation angle by sa for next object in the loop
            angle += sa;
        }
